...
class KNN_classifier():
    ...
    def compute_distance(self, x, t):
        ...
        # print (np.sum((x - t) ** 2))
        return math.sqrt(
            np.sum((x - t) ** 2))  # here are operations with series. Note: make sure to use np.sum to ignore NAN
    ...
    def predict(self, target_observation):
        ...
        df["distance"] = df.apply(self.compute_distance, axis=1,
                                  t=target_observation)  # calculate distanse to each observation
